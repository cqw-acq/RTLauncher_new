...
mod tests {
    ...
    #[test]
    fn clean_param_spaces_strips_inner_whitespace_from_a_double_quoted_param() {
        let input = "\"-DFabricMcEmu= net.minecraft.client.main.Main \"";
        assert_eq!(
            clean_param_spaces(input),
            "-DFabricMcEmu=net.minecraft.client.main.Main"
        );
    }

    #[test]
    fn clean_param_spaces_strips_inner_whitespace_from_a_single_quoted_param() {
        assert_eq!(clean_param_spaces("'foo bar baz'"), "foobarbaz");
    }

    #[test]
    fn clean_param_spaces_only_trims_outer_whitespace_when_unquoted() {
        assert_eq!(clean_param_spaces("  -Xmx1024m  "), "-Xmx1024m");
    }

    #[test]
    fn clean_param_spaces_leaves_mismatched_quotes_untouched_besides_trim() {
        // 只有一端有引号时不应被当作被引号包裹的参数处理。
        assert_eq!(clean_param_spaces("\"-Xmx1024m"), "\"-Xmx1024m");
    }

    #[test]
    fn shell_split_splits_on_plain_whitespace() {
        assert_eq!(
            shell_split("-Xmx1024m -Xms512m"),
            vec!["-Xmx1024m".to_string(), "-Xms512m".to_string()]
        );
    }

    #[test]
    fn shell_split_collapses_repeated_whitespace_and_trims_ends() {
        assert_eq!(
            shell_split("  -Xmx1024m    -Xms512m  "),
            vec!["-Xmx1024m".to_string(), "-Xms512m".to_string()]
        );
    ...
    #[test]
    fn shell_split_keeps_double_quoted_spaces_together_as_one_token() {
        assert_eq!(
            shell_split("-Dfoo=\"hello world\" -Xmx1024m"),
            vec!["-Dfoo=hello world".to_string(), "-Xmx1024m".to_string()]
        );
    }

    #[test]
    fn shell_split_keeps_single_quoted_spaces_together_as_one_token() {
        assert_eq!(
            shell_split("'-Dfoo=hello world'"),
            vec!["-Dfoo=hello world".to_string()]
        );
    }

    #[test]
    fn shell_split_returns_an_empty_vector_for_blank_input() {
        assert!(shell_split("").is_empty());
        assert!(shell_split("   ").is_empty());
    }

    #[test]
    fn library_name_to_path_converts_a_maven_coordinate_to_a_relative_jar_path() {
        assert_eq!(
            library_name_to_path("net.minecraft:launchwrapper:1.12"),
            Some("net/minecraft/launchwrapper/1.12/launchwrapper-1.12.jar".to_string())
        );
    }

    #[test]
    fn library_name_to_path_ignores_a_trailing_classifier_segment() {
        assert_eq!(
            library_name_to_path("org.lwjgl:lwjgl:3.3.1:natives-linux"),
            Some("org/lwjgl/lwjgl/3.3.1/lwjgl-3.3.1.jar".to_string())
        );
    }

    #[test]
    fn library_name_to_path_returns_none_for_incomplete_coordinates() {
        assert_eq!(library_name_to_path("net.minecraft:launchwrapper"), None);
        assert_eq!(library_name_to_path("no-colon-at-all"), None);
    }

    #[test]
    fn parse_library_path_extracts_group_artifact_and_version() {
        assert_eq!(
            parse_library_path("org/apache/commons/commons-lang3/3.3.2/commons-lang3-3.3.2.jar"),
            Some((
    ...
    #[test]
    fn parse_library_path_rejects_a_filename_that_does_not_match_artifact_and_version() {
        assert_eq!(
            parse_library_path("org/apache/commons/commons-lang3/3.3.2/wrong-name.jar"),
            None
        );
    }

    #[test]
    fn parse_library_path_rejects_paths_that_are_too_short_or_missing_the_jar_suffix() {
        assert_eq!(parse_library_path("a/b.jar"), None);
        assert_eq!(
            parse_library_path("org/apache/commons/commons-lang3/3.3.2/commons-lang3-3.3.2"),
            None
        );
    }

    #[test]
    fn compare_versions_detects_a_greater_major_or_minor_component() {
        assert!(compare_versions("1.13.0", "1.12.2"));
        assert!(compare_versions("2", "1.99.99"));
        assert!(!compare_versions("1.12.2", "1.13.0"));
    }

    #[test]
    fn compare_versions_treats_missing_trailing_components_as_zero() {
        assert!(!compare_versions("1.12", "1.12.0"));
        assert!(compare_versions("1.12.1", "1.12"));
    }

    #[test]
    fn compare_versions_returns_false_for_equal_versions() {
        assert!(!compare_versions("1.12.2", "1.12.2"));
    }

    #[test]
    fn detect_minecraft_version_from_patches_finds_the_game_patch_version() {
        let json = json!({
            "patches": [
        ...
        assert_eq!(
            detect_minecraft_version_from_patches(&json),
            Some("1.20.1".to_string())
        );
    }

    #[test]
    fn detect_minecraft_version_from_patches_returns_none_without_a_game_patch() {
        let json = json!({ "patches": [ { "id": "forge", "version": "47.2.0" } ] });
        assert_eq!(detect_minecraft_version_from_patches(&json), None);
    }

    #[test]
    fn detect_minecraft_version_from_patches_returns_none_when_patches_is_missing_or_invalid() {
        assert_eq!(detect_minecraft_version_from_patches(&json!({})), None);
        assert_eq!(
            detect_minecraft_version_from_patches(&json!({ "patches": "not-an-array" })),
            None
        );
    }

    #[test]
    fn launcher_rules_allow_defaults_to_true_when_there_are_no_rules() {
        assert!(launcher_rules_allow(None));
    }

    #[test]
    fn launcher_rules_allow_defaults_to_false_for_an_empty_rule_list() {
        assert!(!launcher_rules_allow(Some(&json!([]))));
    }

    #[test]
    fn launcher_rules_allow_matches_the_current_operating_system() {
        ...
        let matching = json!([{ "action": "allow", "os": { "name": current_os } }]);
        assert!(launcher_rules_allow(Some(&matching)));

        let non_matching = json!([{ "action": "allow", "os": { "name": "some-other-os" } }]);
        assert!(!launcher_rules_allow(Some(&non_matching)));
    }

    #[test]
    fn launcher_rules_allow_lets_a_later_disallow_override_an_earlier_allow() {
        let rules = json!([
            { "action": "allow" },
            { "action": "disallow" }
        ]);
    ...
    #[test]
    fn launcher_rules_allow_treats_an_empty_features_object_as_matching() {
        let rules = json!([{ "action": "allow", "features": {} }]);
        assert!(launcher_rules_allow(Some(&rules)));
    }
}
